...
" Added by slx
" Enhance gotodefine of cpp
function! youcompleteme#GotoDefinitionCpp(...)
    let ret = s:Pyeval("ycm_state.SendCommandRequest(['GoTo'], '', False, 0, 0)")
    if type(l:ret) != 4 || !has_key(l:ret, "filepath")
        return
    endif
    let retfile = split(l:ret["filepath"], "/")[-1]
    let filetype = split(l:retfile, '\.')[-1]
    let filename = split(l:retfile, '\.')[0]
    if filetype != 'h'
        return
    endif

    let pat = l:filename . ".{cc,c,cpp}"
    exec "silent grep -r --include=".l:pat." ".expand('<cword>')." ."
endfunction
